...
rules_texts = {
    ...
    "1_содержание": "Любые попытки обхода правил этого сервера запрещены, также как и их обсуждение. Если вы заметили "
                    "какую-то недоработку или ошибку — сообщите о ней администратору.",

    "2_заголовок": "Уважайте собеседников",
    "2_содержание": "Никаких оскорблений, провокаций, дискриминации, угроз в реальной жизни и прочего "
                    "неконструктивного поведения. Если собеседник нарушает это правило, не нужно отвечать в той же "
                    "манере добавьте нарушителя в игнор и/или обратитесь к модератору или администратору.",

    "3_заголовок": "Общайтесь без мата",
    "3_содержание": "Использование ненормативной лексики, в том числе в завуалированном виде – запрещено. Это также "
                    "касается текста на картинках, аватарках или в именах.",
    ...
    "9_содержание": "В каналах запрещено обсуждение дисциплинарных мер, наложенных как на игровом сервере, так и на "
                    "сервере Discord или другой официальной площадке. Если вы не согласны с полученными "
                    "ограничениями, обратитесь в лс [группы ВКонтакте](http://vk.me/breadixhelp).",
    ...
    "10_содержание": "Не постите «взрослый» или шок-контент. Это касается всего: текста сообщения, эмодзи, картинок, "
                     "информации по ссылке, ваших аватарок или имен. Под запретом не только сексуальные темы, "
                     "но и изображения жестокого обращения с людьми или животными, а также резко неприятные или "
                     "провокационные изображения.",

    "11_заголовок": "Соблюдайте предназнаючение каналов",
    "11_содержание": "В закрепленных сообщениях каждого канала есть описание, для какого рода обсуждений он "
                     "предназначен. Если вы не знаете, к какому каналу относится ваш вопрос, вы можете спросить у "
                     "модератора или администратора.",

    "12_заголовок": "Соблюдайте правила платформы Discord",
    "12_содержание": "Любые действия, нарушающие [правила платформы Discord](https://discord.com/guidelines), "
                     "считаются крайне серьёзным нарушением и наказываются соответствующе.",
}
...
class RulesCog(commands.Cog):
    ...
    async def rule_clause(self, inter: disnake.ApplicationCommandInteraction, пункт: Rules_clauses,
        ...
        clause = пункт
        member = упоминание
        clause_embed = disnake.Embed(title=f"{convert_number(clause)} пункт правил discord сообщества")
        clause_embed.add_field(
            name=rules_texts[str(clause) + '_заголовок'],
            value=rules_texts[str(clause) + '_содержание']
        )
